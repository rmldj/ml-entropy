...
if len(sys.argv)<2:
    print('usage: python dependence_unordered.py nf')
    quit()

nf = int(sys.argv[1])


logging.basicConfig(filename='dependence_unordered_nf_{}.log'.format(nf), format='%(message)s', level=logging.DEBUG)
...
X_not = np.concatenate((X1, X2, X3), axis=1)
X_or = np.concatenate((X1, X2, X4), axis=1)
X_and = np.concatenate((X1, X2, X5), axis=1)
X_xor = np.concatenate((X1, X2, X6), axis=1)
